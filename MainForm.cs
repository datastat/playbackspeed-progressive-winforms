...
namespace SampleFilePlayback
{
    public partial class MainForm : Form
    {
        ...
        class PlayerState
        {
            ...
            public double dblDesiredRate;
            public double dblActualRate;
        ...
        private M_AV_PROPS m_avPropsCurrent;
        ...
        private int currentFrameInVideo = 0;
        ...
        private bool NextFrame()
        {
            ...
            lock (m_playerState.stateLock)
            {
                ...
                m_playerState.dblFrameRequest = -1;


                if (Math.Abs(m_playerState.dblActualRate - m_playerState.dblDesiredRate) > tickValue)
                {
                    if (m_playerState.dblActualRate > m_playerState.dblDesiredRate)
                    {
                        m_playerState.dblActualRate = Math.Round(m_playerState.dblActualRate - tickValue, 3);
                    }
                    else
                    {
                        m_playerState.dblActualRate = Math.Round(m_playerState.dblActualRate + tickValue, 3);
                    ...
                    {
                        currentPlaybackSpeed = $"rate={Math.Abs(currentPlaybackSpeed).ToString().Replace(',', '.')}";
                    }
                    else
                    {
                ...
                strParams += $" rate={Math.Abs(m_playerState.dblActualRate).ToString().Replace(',', '.')}";
            ...
            lock (m_objLock) // For prevent reader replace in OpenFile() and overlay change
            {
                ...
                try
                {
                    if (m_objMFReader != null)
                    {
                        ...
                        //m_objMFReader.SourceFrameGetByTime(dblRequest, -1, out pFrame, strParams);
                        //m_objMFReader.SourceFrameGet(-1, out pFrame, strParams);
            ...
            try
            {
                ...
                pFrame.MFAVPropsGet(out m_avPropsCurrent, out int audioSamples);                
                // update frame gain based on audio meter's values
                
                for (int i = 0; i < mAudioMeter1.arrChannels.Length; i++)
                {
                    if (mAudioMeter1.arrChannels[i] != null)
                        pFrame.MFAudioGain(i.ToString(), mAudioMeter1.arrChannels[i].Gain, mAudioMeter1.arrChannels[i].Gain);
                }
                // Get frame timings
                pFrame.MFTimeGet(out mTime);

                var changeFor = Math.Abs(currentFrameInVideo - mTime.tcFrame.nExtraCounter);

                if (changeFor > 1)
                {
                    Console.WriteLine($"{DateTime.UtcNow} jumped for {currentFrameInVideo - mTime.tcFrame.nExtraCounter}");
                }

                currentFrameInVideo = mTime.tcFrame.nExtraCounter;
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            if (mAudioMeter1 == null)
            {
                mAudioMeter1 = new MAudioMeter();
            }

            // this is null sometimes
        ...
        {
            comboBoxAudio.Items.Add("Custom/No conversion");
            comboBoxAudio.Items.Add("48kHz 2ch 16bit");
            comboBoxAudio.Items.Add("48kHz 16ch 16bit");
            comboBoxAudio.Items.Add("44.1kHz 2ch 16bit");
            comboBoxAudio.Items.Add("44.1kHz 16ch 16bit");
        ...
        {
            switch (comboBoxAudio.SelectedIndex)
            {
                case 0: //"Custom/No conversion"
                    {
                        m_avProps.audProps = new M_AUD_PROPS();
                        break;
                    }
                case 1: //"48kHz 2ch 16bit"
                    {
                        m_avProps.audProps.nSamplesPerSec = 48000;
                        m_avProps.audProps.nChannels = 2;
                        m_avProps.audProps.nBitsPerSample = 16;
                        break;
                    }
                case 2: //"48kHz 16ch 16bit"
                    {
                        m_avProps.audProps.nSamplesPerSec = 48000;
                        m_avProps.audProps.nChannels = 16;
                        m_avProps.audProps.nBitsPerSample = 16;
                        break;
                    }
                case 3: //"44.1kHz 2ch 16bit"
                    {
                        m_avProps.audProps.nSamplesPerSec = 44100;
                        m_avProps.audProps.nChannels = 2;
                        m_avProps.audProps.nBitsPerSample = 16;
                        break;
                    }
                case 4: //"44.1kHz 16ch 16bit"
                    {
                        m_avProps.audProps.nSamplesPerSec = 44100;
                        m_avProps.audProps.nChannels = 16;
                        m_avProps.audProps.nBitsPerSample = 16;
        ...
        private void FillVideoFormats()
        {
            ...
            ComboBoxAudio_SelectedIndexChanged(null, null);
        }
        ...
        private void play()
        {
            lock (m_playerState.stateLock)
            {
                ...
                m_playerState.dblDesiredRate = 1;
        ...
        private void fastFwd()
        {
            lock (m_playerState.stateLock)
            {
                ...
                {
                    m_playerState.dblDesiredRate += 1.0;
                }
                else
                {
                    m_playerState.dblDesiredRate = 2.0;
                    m_playerState.state = eState.eST_PlayFwd;
                }
            }
        }

        private void SetForwardPlayBackSpeed(double playbackSpeed)
        {
            lock (m_playerState.stateLock)
            {
                m_playerState.state = eState.eST_PlayFwd;
                m_playerState.dblDesiredRate = playbackSpeed;
        ...
        private void update_UI(M_TIME mTime)
        {
            ...
            labelStatus.Text = Path.GetFileName(m_playerState.strFileName) + " | PlayerState = " + m_playerState.state + " | Pos = " + (secTime2DTime(dblPos)).ToString("HH:mm:ss.fff") + " / " + (secTime2DTime(m_playerState.dblDuration)).ToString("HH:mm:ss.fff") + "\r\n" +
                "Audio Drops: " + strAudioDrop + "   Audio Dups: " + strAudioDup + "   Fps:" + strFPS + "   Desired Rate:" + m_playerState.dblDesiredRate.ToString("0.00") + "   Actual Rate:" + m_playerState.dblActualRate.ToString("0.00") + "   Decoding mode: " + (String.IsNullOrEmpty(strHWAcc) ? "CPU" : strHWAcc);
            // updateAudioMeter
            mAudioMeter1.AvProps = m_avPropsCurrent;
        ...
        {
            SetForwardPlayBackSpeed(0.5);
        }

        private void button3_Click(object sender, EventArgs e)
        {
            SetForwardPlayBackSpeed(0.25);
        }

        private void button4_Click(object sender, EventArgs e)
        {
            SetForwardPlayBackSpeed(0.99);
        }
    }
}
